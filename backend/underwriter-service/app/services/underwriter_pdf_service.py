"""
InsureAssist Pure-Python PDF Generation Engine for Underwriter Workspace & AI Summaries.
Generates standard-compliant PDF 1.4 binary documents without external dependencies.
"""
...
class SimplePDFBuilder:
    ...
    def add_header(self, title: str, subtitle: str = "INSUREASSIST-GENERATED SUMMARY"):
        self.add_rect(self.margin_left, self.cursor_y - 28.0, self.content_width, 42.0, fill_rgb=(0.20, 0.10, 0.08))
        self.add_text("INSUREASSIST UNDERWRITING SERVICES", font="F2", size=13.0, x=self.margin_left + 14.0, y=self.cursor_y - 4.0, rgb=(1.0, 1.0, 1.0))
        self.add_text(subtitle, font="F1", size=8.5, x=self.margin_left + 14.0, y=self.cursor_y - 20.0, rgb=(0.95, 0.85, 0.80))
        self.cursor_y -= 44.0

        self.add_text(title, font="F2", size=15.0, rgb=(0.20, 0.10, 0.08))
        self.cursor_y -= 18.0
        self.add_line(self.margin_left, self.cursor_y, self.margin_right, self.cursor_y, stroke_rgb=(0.20, 0.10, 0.08), line_width=1.5)
        self.cursor_y -= 14.0

    def add_section_title(self, title: str):
        self.check_space(30.0)
        self.add_rect(self.margin_left, self.cursor_y - 4.0, self.content_width, 18.0, fill_rgb=(0.96, 0.94, 0.92))
        self.add_text(title.upper(), font="F2", size=9.5, x=self.margin_left + 8.0, y=self.cursor_y + 1.0, rgb=(0.20, 0.10, 0.08))
        self.cursor_y -= 18.0
    ...
    def add_key_value_grid(self, items: List[tuple]):
        ...
        for i in range(0, len(items), 2):
            ...
            self.add_text(str(v1 or "N/A"), font="F1", size=9.0, x=self.margin_left + 120.0, y=self.cursor_y, rgb=(0.1, 0.1, 0.1))

            if i + 1 < len(items):
                k2, v2 = items[i+1]
                self.add_text(f"{k2}:", font="F2", size=9.0, x=self.margin_left + col_w + 4.0, y=self.cursor_y, rgb=(0.3, 0.3, 0.3))
                self.add_text(str(v2 or "N/A"), font="F1", size=9.0, x=self.margin_left + col_w + 120.0, y=self.cursor_y, rgb=(0.1, 0.1, 0.1))
    ...
    def add_table(self, headers: List[str], rows: List[List[str]], col_widths: List[float] = None):
        ...
        # Draw Table Header
        self.add_rect(self.margin_left, self.cursor_y - 4.0, self.content_width, header_h, fill_rgb=(0.28, 0.16, 0.12))
        ...
        for r_idx, row in enumerate(rows):
            self.check_space(row_h + 4.0)
            fill = (0.98, 0.97, 0.96) if r_idx % 2 == 1 else (1.0, 1.0, 1.0)
            self.add_rect(self.margin_left, self.cursor_y - 3.0, self.content_width, row_h, fill_rgb=fill, stroke_rgb=(0.90, 0.88, 0.85), line_width=0.5)
            cur_x = self.margin_left
    ...
    def add_disclaimer(self, text: str = None):
        ...
        default_disclaimer = (
            "This is an InsureAssist-generated summary intended to support underwriting review using information available in the system. "
            "Official policy wording, declarations, endorsements, and applicable underwriting authority remain authoritative."
        )
        ...
        self.add_text("NOTE & UNDERWRITING AUTHORITY NOTICE:", font="F2", size=7.5, x=self.margin_left + 8.0, y=self.cursor_y - 2.0, rgb=(0.4, 0.4, 0.4))
    ...
    def compile(self) -> bytes:
        ...
        for p_num, page_ops in enumerate(self.pages):
            ...
            f_op1 = f"BT /F1 8.00 Tf 0.500 0.500 0.500 rg {self.margin_left:.2f} {footer_y:.2f} Td (InsureAssist Underwriting Review | Generated: {ts}) Tj ET"
...
def build_underwriter_application_summary_pdf(app_record: Any, customer: Any, underwriter_user: Optional[Any] = None) -> bytes:
    """
    Generates a professional InsureAssist-generated Application Summary PDF for Underwriting review.
    """
    builder = SimplePDFBuilder()
    builder.add_header(f"Underwriting Application Summary: {app_record.application_id}", subtitle="INSUREASSIST-GENERATED UNDERWRITING REVIEW SUMMARY")

    cust_name = getattr(customer, "name", "Customer Name")
    cust_email = getattr(customer, "email", "N/A")
    uw_name = getattr(underwriter_user, "name", "Underwriting Officer")

    builder.add_section_title("Application & Risk Overview")
    builder.add_key_value_grid([
        ("Application ID", app_record.application_id),
        ("Product Line", getattr(app_record, "product_name", app_record.policy_type)),
        ("Applicant Name", cust_name),
        ("Applicant Email", cust_email),
        ("Application Status", getattr(app_record, "status", "Submitted")),
        ...
        ("Reviewing Officer", uw_name)
    ])

    builder.add_section_title("Coverage Limits & Actuarial Rating")
    ...
    builder.add_key_value_grid([
        ...
        ("Deductible Required", f"${ded_val:,.2f}"),
        ("Estimated Annual Premium", f"${prem_val:,.2f} / year")
    ])
    ...
    if getattr(app_record, "agent_notes", None):
        builder.add_section_title("Intake & Agent Field Notes")
...
def build_underwriter_policy_summary_pdf(policy: Any, customer: Any, coverages: List[Any], exclusions: List[Any], underwriter_user: Optional[Any] = None) -> bytes:
    """
    ...
    if exclusions:
        builder.add_section_title("Named Exclusions & Underwriting Riders")
...
def build_underwriter_claim_summary_pdf(claim: Any, customer: Any, policy: Optional[Any] = None, underwriter_user: Optional[Any] = None) -> bytes:
    """
    ...
    builder.add_section_title("Claim Details & Loss Experience")
    builder.add_key_value_grid([
        ...
        ("Reported Loss Amount", f"${claim_amt:,.2f}" if claim_amt > 0 else "Pending Inspection"),
