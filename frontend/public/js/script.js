const _IA = (typeof window !== 'undefined' && window.INSUREASSIST_API) ? window.INSUREASSIST_API : {};
const AUTH_SERVICE_URL = _IA.auth || 'http://127.0.0.1:8001';
const CUSTOMER_SERVICE_URL = _IA.customer || 'http://127.0.0.1:8002';
const AGENT_SERVICE_URL = _IA.agent || 'http://127.0.0.1:8003';
const UNDERWRITER_SERVICE_URL = _IA.underwriter || 'http://127.0.0.1:8004';
const ADMIN_SERVICE_URL = _IA.admin || 'http://127.0.0.1:8005';
const AI_SERVICE_URL = _IA.ai || 'http://127.0.0.1:8006';
...
function bootInsureAssist() {
  if (window.__insureAssistBooted) return;
  window.__insureAssistBooted = true;
  ...
  const loginUrl = `${AUTH_SERVICE_URL}/auth/login`;
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', bootInsureAssist);
} else {
  bootInsureAssist();
}
