...
function getAuthToken() {
  if (typeof window !== 'undefined' && window.CURRENT_AUTH) {
    if (window.CURRENT_AUTH.access_token) return window.CURRENT_AUTH.access_token;
    if (window.CURRENT_AUTH.token) return window.CURRENT_AUTH.token;
  }
  const storages = [
  ...
  for (const s of storages) {
    if (!s) continue;
    const direct = s.getItem('auth_token') || s.getItem('access_token') || s.getItem('jwt');
    if (direct) return direct;
    const authJson = s.getItem('insureassist_auth') || s.getItem('auth');
    if (authJson) {
      try {
...
let chatHistory = {
  customer: [{ sender: 'bot', text: 'Hello! I am your AI Policy Assistant. How can I help you with your insurance policies today?' }],
  agent: [{ sender: 'bot', text: 'Hello! I am your Client & Policy Support AI Assistant. How can I assist you with your assigned portfolio today?' }],
...
function getMockChatResponse(query, role) {
  const normRole = (role || 'customer').toString().toLowerCase();
  if (['customer', 'agent', 'broker', 'underwriter', 'admin'].includes(normRole)) {
    return 'I’m unable to retrieve that information right now. Please try again.';
  }
  return 'I’m unable to retrieve that information right now. Please try again.';
}
...
async function downloadOrViewDoc(docName, docRef) {
  ...
  const cleanName = (docName || 'Document').replace(/[^\w\s\-\.]/g, '').trim();
  const cleanRef = (docRef || '').trim();
  const currentRole = (typeof MOCK_DB !== 'undefined' && MOCK_DB.currentRole) ? MOCK_DB.currentRole : 'customer';
  ...
  try {
    ...
    if (currentRole === 'agent') {
      ...
      if (cleanRef) params.append('ref_id', cleanRef);
      params.append('doc_type', cleanName);
      downloadUrl = `${AGENT_SERVICE_URL}/agent/documents/download?${params.toString()}`;
    } else {
      const params = new URLSearchParams();
      if (cleanRef) params.append('ref', cleanRef);
      if (cleanName) params.append('name', cleanName);
      downloadUrl = `${CUSTOMER_SERVICE_URL}/customer/documents/download?${params.toString()}`;
    }
    ...
    if (!res.ok) {
      if (res.status === 404 || res.status === 403) {
        showToast('Document summary currently unavailable.', 'error');
        return;
      }
      const errData = await res.json().catch(() => ({}));
      showToast(errData.detail || 'Document summary currently unavailable.', 'error');
      return;
    }
    ...
    if (!filename) {
      filename = cleanRef.toLowerCase().endsWith('.pdf') ? cleanRef : `InsureAssist_${(cleanRef || cleanName).replace(/\s+/g, '_')}.pdf`;
    }
...
function attachChatEventListeners() {
  ...
  const sendMessage = async () => {
    ...
    const normRole = (role || (typeof MOCK_DB !== 'undefined' && MOCK_DB.currentRole) || 'customer').toString().toLowerCase();
    chatHistory[normRole] = chatHistory[normRole] || [];
    chatHistory[normRole].push({ sender: 'user', text: text });
    if (input) input.value = '';

    if (normRole === 'customer') {
      // Add typing indicator
      chatHistory[normRole].push({
        sender: 'bot',
      ...
      try {
        ...
        const typingIdx = chatHistory[normRole].findIndex(m => m.isTyping);
        if (typingIdx !== -1) chatHistory[normRole].splice(typingIdx, 1);

        if (res.ok) {
          const data = await res.json();
          const botReply = data.response || 'No response received from AI service.';
          chatHistory[normRole].push({ sender: 'bot', text: botReply });
          openAIChatSlidePanel();
          return;
        } else {
          chatHistory[normRole].push({ sender: 'bot', text: 'AI Assistant is temporarily unavailable. Please try again.' });
          openAIChatSlidePanel();
          return;
        }
      } catch (err) {
    ...
    } else if (['agent', 'broker'].includes(normRole)) {
      // Add typing indicator
      chatHistory[normRole].push({
        sender: 'bot',
        text: '<div class="chat-typing-indicator"><span></span><span></span><span></span></div>',
      ...
      try {
        ...
        const res = await fetch(`${AGENT_SERVICE_URL}/agent/chat/messages`, {
          method: 'POST',
          headers: headers,
          body: JSON.stringify({
            message: text
        ...
        if (res.ok) {
          ...
          chatHistory[normRole].push({ sender: 'bot', text: 'Agent AI is temporarily unavailable. Please try again.' });
          openAIChatSlidePanel();
          return;
        }
      } catch (err) {
        console.warn('Slide panel Agent AI call error:', err);
        const typingIdx = chatHistory[normRole].findIndex(m => m.isTyping);
        if (typingIdx !== -1) chatHistory[normRole].splice(typingIdx, 1);
        chatHistory[normRole].push({ sender: 'bot', text: 'Agent AI is temporarily unavailable. Please try again.' });
        openAIChatSlidePanel();
      ...
      try {
        const headers = (typeof getAuthHeaders === 'function') ? getAuthHeaders() : { 'Content-Type': 'application/json' };
        const res = await fetch(`${UNDERWRITER_SERVICE_URL}/underwriter/chat/messages`, {
          method: 'POST',
          headers: headers,
          body: JSON.stringify({
            message: text
        ...
        if (res.ok) {
          ...
          chatHistory[normRole].push({ sender: 'bot', text: 'Underwriter AI is temporarily unavailable. Please try again.' });
          openAIChatSlidePanel();
          return;
        }
      } catch (err) {
        console.warn('Slide panel Underwriter AI call error:', err);
        const typingIdx = chatHistory[normRole].findIndex(m => m.isTyping);
        if (typingIdx !== -1) chatHistory[normRole].splice(typingIdx, 1);
        chatHistory[normRole].push({ sender: 'bot', text: 'Underwriter AI is temporarily unavailable. Please try again.' });
        openAIChatSlidePanel();
      ...
      try {
        const headers = (typeof getAuthHeaders === 'function') ? getAuthHeaders() : { 'Content-Type': 'application/json' };
        const res = await fetch(`${ADMIN_SERVICE_URL}/admin/chat/messages`, {
          method: 'POST',
          headers: headers,
          body: JSON.stringify({
            message: text
        ...
        if (res.ok) {
          ...
          chatHistory[normRole].push({ sender: 'bot', text: 'Admin Governance AI is temporarily unavailable. Please try again.' });
          openAIChatSlidePanel();
          return;
        }
      } catch (err) {
        console.warn('Slide panel Admin AI call error:', err);
        const typingIdx = chatHistory[normRole].findIndex(m => m.isTyping);
        if (typingIdx !== -1) chatHistory[normRole].splice(typingIdx, 1);
        chatHistory[normRole].push({ sender: 'bot', text: 'Admin Governance AI is temporarily unavailable. Please try again.' });
        openAIChatSlidePanel();
    ...
    setTimeout(() => {
      const reply = getMockChatResponse(text, normRole);
      chatHistory[normRole].push({ sender: 'bot', text: reply });
      openAIChatSlidePanel();
    }, 450);
  };
...
async function handleChatPromptClick(promptText) {
  ...
  const normRole = (role || 'customer').toString().toLowerCase();
  chatHistory[normRole] = chatHistory[normRole] || [];
  chatHistory[normRole].push({ sender: 'user', text: promptText });
  ...
  if (normRole === 'customer') {
    ...
    try {
      ...
      console.warn('Chat prompt Agent AI call error:', err);
      const typingIdx = chatHistory[normRole].findIndex(m => m.isTyping);
      if (typingIdx !== -1) chatHistory[normRole].splice(typingIdx, 1);
      chatHistory[normRole].push({ sender: 'bot', text: 'Agent AI is temporarily unavailable. Please try again.' });
      openAIChatSlidePanel();
    ...
    try {
      ...
      console.warn('Chat prompt Underwriter AI call error:', err);
      const typingIdx = chatHistory[normRole].findIndex(m => m.isTyping);
      if (typingIdx !== -1) chatHistory[normRole].splice(typingIdx, 1);
      chatHistory[normRole].push({ sender: 'bot', text: 'Underwriter AI is temporarily unavailable. Please try again.' });
      openAIChatSlidePanel();
    ...
    try {
      ...
      console.warn('Chat prompt Admin AI call error:', err);
      const typingIdx = chatHistory[normRole].findIndex(m => m.isTyping);
      if (typingIdx !== -1) chatHistory[normRole].splice(typingIdx, 1);
      chatHistory[normRole].push({ sender: 'bot', text: 'Admin Governance AI is temporarily unavailable. Please try again.' });
      openAIChatSlidePanel();
  ...
  setTimeout(() => {
    const reply = getMockChatResponse(promptText, normRole);
    chatHistory[normRole].push({ sender: 'bot', text: reply });
    openAIChatSlidePanel();
  }, 400);
};
...
async function loadCustomerChatHistoryFromBackend() {
  try {
    ...
    if (!res.ok) {
      console.warn('Unable to load customer chat conversations from backend:', res.status);
      return;
    }
...
/**
 * Loads persistent Agent AI chat conversations from PostgreSQL.
 */
async function loadAgentChatHistoryFromBackend() {
  try {
    const headers = (typeof getAuthHeaders === 'function') ? getAuthHeaders() : { 'Content-Type': 'application/json' };
    const res = await fetch(`${AGENT_SERVICE_URL}/agent/chat/conversations`, {
      method: 'GET',
      headers: headers
    });

    if (!res.ok) {
      console.warn('Unable to load agent chat conversations from backend:', res.status);
      return;
    }

    const conversations = await res.json();
    const data = getRoleAiData('agent');
    ...
    if (data.conversations.length > 0) {
      ...
      await fetchConversationMessages('agent', data.activeConversationId);
    } else {
      data.activeConversationId = null;
      renderRoleChatMessages('agent', null);
    }
    renderRoleChatHistory('agent');
  } catch (err) {
    console.error('Error loading agent chat history:', err);
  }
}
window.loadAgentChatHistoryFromBackend = loadAgentChatHistoryFromBackend;

/**
 * Loads persistent Underwriter AI chat conversations from PostgreSQL.
 */
async function loadUnderwriterChatHistoryFromBackend() {
  try {
    const headers = (typeof getAuthHeaders === 'function') ? getAuthHeaders() : { 'Content-Type': 'application/json' };
    const res = await fetch(`${UNDERWRITER_SERVICE_URL}/underwriter/chat/conversations`, {
      method: 'GET',
      headers: headers
    });

    if (!res.ok) {
      console.warn('Unable to load underwriter chat conversations from backend:', res.status);
      return;
    }

    const conversations = await res.json();
    const data = getRoleAiData('underwriter');
    ...
    if (data.conversations.length > 0) {
      ...
      await fetchConversationMessages('underwriter', data.activeConversationId);
    } else {
      data.activeConversationId = null;
      renderRoleChatMessages('underwriter', null);
    }
    renderRoleChatHistory('underwriter');
  } catch (err) {
    console.error('Error loading underwriter chat history:', err);
  }
}
window.loadUnderwriterChatHistoryFromBackend = loadUnderwriterChatHistoryFromBackend;

/**
 * Loads persistent Admin AI chat conversations from PostgreSQL.
 */
async function loadAdminChatHistoryFromBackend() {
  try {
    const headers = (typeof getAuthHeaders === 'function') ? getAuthHeaders() : { 'Content-Type': 'application/json' };
    const res = await fetch(`${ADMIN_SERVICE_URL}/admin/chat/conversations`, {
      method: 'GET',
      headers: headers
    });

    if (!res.ok) {
      console.warn('Unable to load admin chat conversations from backend:', res.status);
      return;
    }

    const conversations = await res.json();
    const data = getRoleAiData('admin');
    ...
    if (data.conversations.length > 0) {
      ...
      await fetchConversationMessages('admin', data.activeConversationId);
    } else {
      data.activeConversationId = null;
      renderRoleChatMessages('admin', null);
    }
    renderRoleChatHistory('admin');
  } catch (err) {
    console.error('Error loading admin chat history:', err);
  }
}
window.loadAdminChatHistoryFromBackend = loadAdminChatHistoryFromBackend;
...
async function fetchConversationMessages(role, conversationId) {
  ...
  const normRole = (role || 'customer').toString().toLowerCase();
  if (!['customer', 'agent', 'broker', 'underwriter', 'admin'].includes(normRole)) return;

  const data = getRoleAiData(normRole);
  const conv = data.conversations.find(c => c.conversation_id === conversationId);
  if (!conv) return;

  const url = normRole === 'underwriter'
    ? `${UNDERWRITER_SERVICE_URL}/underwriter/chat/conversations/${conversationId}/messages`
    : normRole === 'admin'
    ? `${ADMIN_SERVICE_URL}/admin/chat/conversations/${conversationId}/messages`
    : ['agent', 'broker'].includes(normRole)
    ? `${AGENT_SERVICE_URL}/agent/chat/conversations/${conversationId}/messages`
    : `${CUSTOMER_SERVICE_URL}/customer/chat/conversations/${conversationId}/messages`;
  ...
  try {
    ...
    if (res.ok) {
      ...
      renderRoleChatMessages(normRole, conversationId);
    }
  } catch (err) {
...
function renderRoleChatMessages(role, conversationId) {
  ...
  const roleDefaults = {
    ...
    agent: { title: 'Agent AI Assistant', avatar: 'AR', welcomeTitle: 'How can I help with your work today?', welcomeDesc: 'Ask about applications, customers, policies, claims, documents, or verification.' },
...
async function selectRoleChatConversation(role, convId) {
  ...
  renderRoleChatHistory(normRole);
  if (['customer', 'agent', 'broker', 'underwriter', 'admin'].includes(normRole)) {
    const conv = data.conversations.find(c => c.conversation_id === convId);
    if (!conv || !conv.messages || conv.messages.length === 0) {
      await fetchConversationMessages(normRole, convId);
    } else {
      renderRoleChatMessages(normRole, convId);
    }
  } else {
    renderRoleChatMessages(normRole, convId);
  }
...
async function sendRoleChatMessage(role, customText = null) {
  const normRole = (role || (typeof MOCK_DB !== 'undefined' && MOCK_DB.currentRole) || 'customer').toString().toLowerCase();
  console.log("AI runtime role:", normRole);
  const currentUser = (window.CURRENT_AUTH && window.CURRENT_AUTH.user) || (typeof MOCK_DB !== 'undefined' ? MOCK_DB.customer : null);
  console.log("AI current user:", currentUser);

  const input = document.getElementById(`${normRole}-page-chat-input`) || document.getElementById(`${role}-page-chat-input`);
  ...
  if (['customer', 'agent', 'broker', 'underwriter', 'admin'].includes(normRole)) {
    ...
    renderRoleChatMessages(normRole, conv.conversation_id);

    try {
      const headers = (typeof getAuthHeaders === 'function') ? getAuthHeaders() : { 'Content-Type': 'application/json' };
      const targetConvId = (conv.conversation_id && !conv.conversation_id.startsWith('temp-')) ? conv.conversation_id : null;
      
      const endpoint = ['agent', 'broker'].includes(normRole)
        ? `${AGENT_SERVICE_URL}/agent/chat/messages`
        : normRole === 'underwriter'
        ? `${UNDERWRITER_SERVICE_URL}/underwriter/chat/messages`
        : normRole === 'admin'
        ? `${ADMIN_SERVICE_URL}/admin/chat/messages`
        : `${CUSTOMER_SERVICE_URL}/customer/chat/messages`;
      ...
      console.log("AI CHAT REQUEST →", endpoint, payload);
      ...
      if (res.ok) {
        ...
        console.log("AI CHAT RESPONSE →", res.status, chatData);
        ...
        console.warn("AI CHAT RESPONSE ERROR →", res.status, errJson);
        const errMsg = ['agent', 'broker'].includes(normRole)
          ? 'Agent AI is temporarily unavailable. Please try again.'
          : normRole === 'underwriter'
          ? 'Underwriter AI is temporarily unavailable. Please try again.'
          : normRole === 'admin'
          ? 'Admin Governance AI is temporarily unavailable. Please try again.'
          : 'AI Assistant is temporarily unavailable. Please try again.';
        showToast(errJson.detail || errMsg, 'error');
        const botMsg = {
          message_id: `msg-err-${Date.now()}`,
          conversation_id: conv.conversation_id,
          sender: 'bot',
          content: errMsg,
          timestamp: formatChatTimestamp(new Date())
  ...
  // Non-customer/non-agent roles fallback (e.g. Underwriter / Admin prototype assistance)
  if (!data.activeConversationId) {
    const newId = `conv-${normRole}-${Date.now()}`;
    const newTitle = text.length > 32 ? text.substring(0, 29) + '...' : text;
    const newConv = {
  ...
  setTimeout(() => {
    const botReply = getMockChatResponse(text, normRole);
    const botMsg = {
...
function switchRole(role, targetPage = null) {
  ...
  if (normalizedRole === 'admin') {
    ...
    fetchAdminAudit();
    fetchNotifications();
    loadAdminChatHistoryFromBackend();
    ...
    fetchUnderwriterQueue();
    fetchNotifications();
    loadUnderwriterChatHistoryFromBackend();
    navigateTo(pageToOpen);
